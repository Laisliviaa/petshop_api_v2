...
public class RateLimitConfig {
    ...
    private static final long READ_REFILL_SECS  = 30;
    private static final int  WRITE_CAPACITY    = 5;
    private static final long WRITE_REFILL_SECS = 30;
    ...
    private Bucket buildBucket(boolean write) {
        Bandwidth limit = write
                ? Bandwidth.builder().capacity(WRITE_CAPACITY)
                        .refillGreedy(WRITE_CAPACITY, Duration.ofSeconds(WRITE_REFILL_SECS)).build()
                : Bandwidth.builder().capacity(READ_CAPACITY)
                        .refillGreedy(READ_CAPACITY, Duration.ofSeconds(READ_REFILL_SECS)).build();
